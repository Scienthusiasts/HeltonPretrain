...
class FCOSAssigner(nn.Module):
    ...
    def forward(self, gt_boxes, gt_labels):
        ...
        for level in range(len(self.strides)):
            feat_w = self.img_size[0] // self.strides[level]
            feat_h = self.img_size[1] // self.strides[level]
        ...
        cls_targets_all_level = torch.cat(cls_targets_all_level, dim=1).reshape(-1, 1)
        cnt_targets_all_level = torch.cat(cnt_targets_all_level, dim=1).reshape(-1, 1)
        reg_targets_all_level = torch.cat(reg_targets_all_level, dim=1).reshape(-1, 4)
