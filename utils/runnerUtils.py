...
def recoardArgs(
    ...
    if mode == 'epoch':
        argsHistory.record('val_acc', acc)
        argsHistory.record('val_mAP', mAP)
        argsHistory.record('val_mF1Score', mF1Score)
        argsHistory.saveRecord()
...
def printLog(
    ...
    if mode != 'epoch':
        ...
        if step % log_interval == 0 and mode == 'train':
            ...
            acc_list = argsHistory.args_history_dict['val_acc']
            mAP_list = argsHistory.args_history_dict['val_mAP']
            mF1Score_list = argsHistory.args_history_dict['val_mF1Score']
            # 找到最高准确率对应的epoch
            best_epoch = acc_list.index(max(acc_list)) + 1
            logger.info('=' * 150)
            log = ("Epoch  [%d]  val_acc.: %.5f  mAP: %.5f  mF1Score: %.5f  best_val_epoch: %d" % (epoch+1, acc_list[-1], mAP_list[-1], mF1Score_list[-1], best_epoch))
            logger.info(log)
            logger.info('=' * 150)
...
def saveCkpt(
    ...
    AP50_list = argsHistory.args_history_dict['val_acc']
    if epoch == AP50_list.index(max(AP50_list)):
        torch.save(model.state_dict(), os.path.join(log_dir, 'best_acc.pt'))
        logger.info('best checkpoint(acc) has saved !')
